...
function OfferCard({
  ...
  return h(
    ...
    cardState !== 'initial' && cardState !== 'revealing'
      ? h(
          ...
          h(
            ...
            h('s-stack', { gap: 'none', alignItems: 'center' },
    ...
    cardState === 'revealing'
      ? h('s-box', { padding: 'small' }, h('s-text', { color: 'subdued' }, 'Generating your discount code...'))
      : null,

    cardState === 'error'
      ? h('s-box', { padding: 'small' }, h('s-text', { tone: 'warning' }, `Error: ${errorMessage || 'Failed to load code'}`))
      : null,
