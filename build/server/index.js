...
const loader$d = async ({
  ...
  const whereClause = {
    shopifyDiscountGid: {
      startsWith: "gid://recip/DiscountCodePlaceholder/"
    },
    state: "POOL",
    ...shop ? {
      toShop: {
...
function buildOfferText(discountType, discountValue, newCustomersOnly) {
  const amount = discountType === "FIXED" ? `£${discountValue % 1 === 0 ? discountValue : discountValue.toFixed(2)}` : `${discountValue}%`;
  return newCustomersOnly ? `Get ${amount} off your first order` : `Get ${amount} off`;
}
const loader$8 = async ({
  ...
  const offers = destinationShops.map((shop) => {
    ...
    const newCustomersOnly = ((_c = shop.settings) == null ? void 0 : _c.newCustomersOnly) ?? false;
    return {
      offerId: `offer-${shop.id}`,
      toShopId: shop.id,
      toShopDomain: shop.shopDomain,
      brand: ((_d = shop.settings) == null ? void 0 : _d.brandName) || shop.shopDomain.replace(".myshopify.com", ""),
      description: ((_e = shop.settings) == null ? void 0 : _e.brandDescription) || "Partner offer",
      offer: buildOfferText(discountType, discountValue, newCustomersOnly),
      discountType,
      discountValue,
      logoUrl: ((_f = shop.settings) == null ? void 0 : _f.logoUrl) || void 0
    };
  });
...
const routes = {
  ...
  "routes/api.admin.reset-pool": {
    id: "routes/api.admin.reset-pool",
    parentId: "root",
    path: "api/admin/reset-pool",
    index: void 0,
    caseSensitive: void 0,
    module: route6
  },
